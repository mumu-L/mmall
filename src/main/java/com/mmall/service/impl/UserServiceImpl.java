...
public class UserServiceImpl implements IUserService {
    ...
    public ServerResponse<String> register(User user){
        ...
        ServerResponse validUserName = this.checkValid(user.getUsername(), Const.USERNAME);
        if(!validUserName.isSuccess()){
            return ServerResponse.createByErrorMessage("用户名已存在");
        }
        ServerResponse validEmail = this.checkValid(user.getEmail(), Const.EMAIL);
    ...
    public  ServerResponse<String> forgetResetPassword(String username, String passwordNew, String forgetToken){
        ...
        if(StringUtils.equals(forgetToken,token)){
            ...
            return ServerResponse.createByErrorMessage("token错误，请重新获取后修改密码");
        }
    ...
    public  ServerResponse<User> updateInformation(User user){
        //username是不能被更新的
        //emai进行校验，校验新的email是不是已经存在，并且存在的email如果相同，不能是我们当前的这个用户的
